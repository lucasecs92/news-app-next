...
import { NEWS_API_PROXY_URL, COUNTRY, CATEGORY_TECHNOLOGY } from "../utils/config";
...
const Technology: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchTechnologyNews = async () => {
      ...
      setError(null);
      try {
        const cacheKey = `newsData_${CATEGORY_TECHNOLOGY}`;

        // Lógica de cache em desenvolvimento
        if (process.env.NODE_ENV === "development") {
          const cachedNews = sessionStorage.getItem(cacheKey);
          if (cachedNews) {
            console.log(`🗂️ [${CATEGORY_TECHNOLOGY}] Usando dados do cache (sessionStorage).`);
            if (!ignore) {
        ...
        const response = await fetch(
          `${NEWS_API_PROXY_URL}/${CATEGORY_TECHNOLOGY}?country=${COUNTRY}`
        );
        ...
        if (!ignore) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(`📡 [${CATEGORY_TECHNOLOGY}] Buscando do Proxy e salvando no cache (sessionStorage).`);
        ...
        if (!ignore) {
          console.error(`Erro ao buscar notícias de ${CATEGORY_TECHNOLOGY}: `, err);
          const message =
            err instanceof Error
              ? err.message
              : `Não foi possível carregar as notícias de ${CATEGORY_TECHNOLOGY}. Tente novamente mais tarde.`;
          setError(message);
  ...
  return (
    <section id="technology-content">
      ...
      {articles.length > 0 ? (
        ...
        <p className={styles.noResultsMessage}>Nenhuma notícia de Tecnologia encontrada.</p> // Mensagem aprimorada
      )}
    </section>
  );
};
