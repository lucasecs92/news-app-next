...
const Health: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchHealthNews = async () => {
      ...
      setError(null);
      try {
        const cacheKey = `newsData_${CATEGORY_HEALTH}`;
        ...
        if (!ignore) {
          console.error(`Erro ao buscar notícias de ${CATEGORY_HEALTH}: `, err);
          const message =
            err instanceof Error
              ? err.message
              : `Não foi possível carregar as notícias de ${CATEGORY_HEALTH}. Tente novamente mais tarde.`;
          setError(message);
  ...
  if (loading) return <p className={styles.loadingMessage}>Carregando notícias de Saúde...</p>;
